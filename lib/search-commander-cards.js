...
async function buildDeckContext(options) {
  const progress = createProgress('deck-context', 4, { every: 1 });
  progress.start(`data=${options.dataFile}`);
  const data = JSON.parse(await fs.promises.readFile(options.dataFile, 'utf8'));
  const cards = Array.isArray(data.cards) ? data.cards : [];
  progress.tick(1, `loaded=${cards.length}`);
  const preparedCards = cards.map(prepareCard);
  progress.tick(2, 'prepared');
  ...
  progress.tick(3, `scored suggestions=${suggestions.length}`);
  ...
  progress.tick(4, 'summary');
  progress.done(`suggestions=${suggestions.length}`);
  return context;
}
...
function scoreCards(cards, query) {
  ...
  const scored = [];
  const progress = createProgress('deck-context-score-cards', cards.length);
  progress.start();

  for (let index = 0; index < cards.length; index++) {
    const card = cards[index];
    if (!card.is_commander_legal || card.id === query.commander?.id) {
      progress.tick(index + 1, `scored=${scored.length}`);
      continue;
    }
  ...
  progress.done(`scored=${scored.length}`);
