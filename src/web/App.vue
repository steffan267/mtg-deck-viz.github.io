...
import RoleLegend from './components/sidebar/RoleLegend.vue'
...
import type { CandidateCard, DeckGraph, DeckPayloadEntry, DeckMetrics, GraphNode, MetricsModule, MetricItem, RankGroup, RoleLegendItem, ScoreSection, SignalBar } from './types'
...
const roleVisibility = reactive<Record<string, boolean>>({})
...
const roles = computed<RoleLegendItem[]>(() => {
  const counts = new Map<string, number>()
  for (const node of cardNodes.value) counts.set(String(node.role), (counts.get(String(node.role)) || 0) + (node.qty || 1))
  return [...counts.entries()].sort((a, b) => b[1] - a[1]).map(([role, count]) => {
    if (roleVisibility[role] == null) roleVisibility[role] = true
    return { id: role, label: ROLE_LABELS[role] || role, color: ROLE_COLORS[role] || '#888', count, enabled: roleVisibility[role] !== false }
  })
})
...
function enrichScoreSections(sections: ScoreSection[]): ScoreSection[] {
  return sections.map((section) => {
    if (section.id === 'win') {
      return { ...section, subtitle: section.summary || 'Speed, tutors, card flow, answers, closure, and resilience.', signals: (section.signals || []).map(signal => withSignalTitle(withSelectableFamily(signal), 'win')) }
    }
...
function toggleRole(role: string) {
  roleVisibility[role] = roleVisibility[role] === false
}
    ...
    <div v-if="currentPage === 'visualization'" class="app">
      <SidebarShell :title="activeDeck?.title || 'Deck Interaction Map'" subtitle="Edges = real mechanical interactions. Hover a card to trace its web · click for detail.">
        ...
        <div class="role-scroll">
          <RoleLegend :roles="roles" subtitle="Click to toggle graph visibility" @toggle-role="toggleRole" />
        </div>
      </SidebarShell>
