...
describe('Vue app browser smoke', () => {
  it('loads graph UI, imports via dropped file and paste, compares decks, opens help/detail/recommendations, and resets selection', async () => {
    ...
    expect(wrapper.text()).toContain('Win tuning')
    expect(wrapper.text()).toContain('P95')
    await wrapper.findAll('button').find(button => button.text().includes('Win tuning') && button.text().includes('91'))!.trigger('click')
    await nextTick()
    expect(wrapper.text()).toContain('Your value: 91')
    await wrapper.findAll('button').find(button => button.text() === 'Decklist')!.trigger('click')
    await nextTick()
    expect(wrapper.text()).toContain('Decklist')
