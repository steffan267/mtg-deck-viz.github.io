...
describe('App score breakdown drawers', () => {
  ...
  it('opens a visual bracket comparison for the active deck score', async () => {
    ...
    expect(document.body.textContent).toContain('Compare weighted score to brackets')
    expect(document.body.textContent).toContain('Your weighted score')
    expect(document.body.textContent).toContain('Closest public-deck benchmark')
    expect(document.body.textContent).toContain('65% win · 25% self · 10% cohesion')
    expect(document.body.textContent).toContain('median 74')
    ...
    expect(document.querySelector('.bracket-compare__marker')?.textContent).toContain('You 83')
    expect(document.querySelectorAll('.bracket-compare__median')).toHaveLength(5)
    expect(document.querySelector('.bracket-compare__median[title*="B5 weighted median"]')?.textContent).toContain('B5 med')
